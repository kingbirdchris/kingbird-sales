...
// Returns phase multipliers and additional signals from Claude Haiku
...
A prospect has described their project. A rule-based estimator has already produced hour estimates per phase, adjusted for AI-assisted senior engineers. Your job is to read the description and identify signals that the parameter-based rules would not capture — things like unusual domain complexity, mentioned third-party systems with known integration difficulty, regulatory specifics, or scope that is clearly larger or smaller than the parameters suggest.
...
Rules:
- Set a phase multiplier only when the description contains a clear, specific signal for that phase
...
- additional_questions: maximum 2, only questions specific to what the description raised that the standard parameter set does not already ask`;
  ...
  try {
    ...
    // Extract JSON — Claude should return clean JSON but extract robustly
